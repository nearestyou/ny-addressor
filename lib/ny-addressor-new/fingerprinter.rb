...
require 'digest'
require "countries"
require_relative '../ny-addressor-new/utils'
module NYAddressorNEW
  module Fingerprinter
    ...
    def construct(parts, opts = {})
      required = {
        house_number: NYAddressorNEW.first_present(parts[:house_number], parts[:house], parts[:po_box]),
        street_name: NYAddressorNEW.first_present(parts[:street_name], parts[:suburb]),
        city: NYAddressorNEW.first_present(parts[:city], parts[:city_district], parts[:state], parts[:postcode], parts[:country])
      }
